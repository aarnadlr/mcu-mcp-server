...
- `src/` — TypeScript entrypoint `index.ts` wires Express transport; `create-server.ts` defines weather tools and helpers.
...
- Keep Express setup in `index.ts`; expand tool logic into dedicated modules under `src/tools/` if functionality grows, exporting through `index.ts`.
